...
class BCQ:
    ...
    def predict(self, state):
        ...
        with torch.no_grad():
            # TODO 1.3: Select action from constrained set (using threshold)
            q_values, imts = self.Q_net.forward(state)
            # We used log-softmax in the network construction
            imts = imts.exp()

            # should contain flag for each action (i.e. self.batch_size x 2 - dimensional)
            # whether it is in constrained neighborhood or not.
            imts = None
    ...
    def train(self, offline_buffer, trainsteps):
        ...
        for trainstep in tqdm(range(trainsteps)):
            ...
            # TODO 1.1: Sample data from offline buffer
            ...
            with torch.no_grad():
                ...
                # TODO 1.2: compute target Q-values
...
def parse():
    ...
    parser.add_argument('--pre_train', '-pre', type=int, default=30000)
    ...
    parser.add_argument('--threshold', '-thres', type=float, default=0.3)
...
if __name__ == '__main__':
    ...
    bcq = BCQ(_env, threshold=_args.threshold, learning_rate=_args.learning_rate, gamma=_args.gamma)
    bcq.train(data, trainsteps=_args.train)
